const { When, Then, setDefaultTimeout, Before } = require('cucumber');
    ...
    browser.get('').then(() => {
        LoginHelper.checkLoginPage().then(value => {
            ...
            LoginHelper.tapToButton('signup', 'login').then(() => {
                GeneralHelper.elementExists('page-signup').then(exists => {
...
Then(/^I see "([^"]*)" page$/, (id, callback) => {
    GeneralHelper.elementExists('page-' + id).then(exists => {
        expect(exists).to.equal(true);
        callback();
    });
});
